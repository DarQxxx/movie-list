...
export default function CommentCreate (params) {
  ...
  const userProps = useSelector(state => state.userData)
    ...
    function handleSend(){
      ...
      getCol(params.params.movieId).doc(params.comments.length.toString()).set({
  ...
  return (
    <div className='commentCreate' > 
      {!isReply && (
        ...
        >
          {isLogged ? (<div           onClick={() => {
            setIsReply(true);
            
          }} className="commentCreate__addComment">Dodaj nowy komentarz...</div>) : (<div><span className="commentCreate__login">Zaloguj się</span>, żeby dodać komentarz</div>)}
        </div>
      )}
    {isReply && (
